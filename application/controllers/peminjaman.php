...
class Peminjaman extends CI_Controller{
    ...
    function index(){
		...
		$hasil = $this->m_peminjaman->cariTipe($this->session->userdata('username'));
		
		$data['tipe'] = $hasil[0]->tipe_pegawai;
		
        $data['title']="Booking Kamar";
        $data['tgl_statusawal']=date('Y-m-d');
		$data['tgl_statusakhir']=date('Y-m-d');
        $data['tgl_statusakhir'] = strtotime('+7 day',strtotime($data['tanggalpinjam']));
